    ...
    async with LLM_SEMAPHORE:
        for attempt, tokens in enumerate((max_tokens, max_tokens * 2)):
            ...
            if text.strip() and not (reject_truncated and truncated):
                # Live Issue 5 — measured at rev. 51 (Run 33) and confirmed working: Anthropic
                # shows the expected write-then-read cache pattern, and the worker/compiler
                # prefixes (incl. data_profile) hit cache on reuse. usage is on every response
                # regardless of provider/cache_prefix use, so log it unconditionally rather than
                # special-casing cached calls — a 0 here is itself the data point for an uncached
                # call, and cache_read_input_tokens/cache_creation_input_tokens default to None
                # (not a missing attribute) if a provider's response omits them entirely, so this
                # is safe on DeepSeek's Anthropic-Messages-API-compatible endpoint too, which is
