...
console anyway when it happens (DIVERGER_PLAN.md's Live Issue 29 entry is explicit about this).
...
def _describe_status_error(e: anthropic.APIStatusError) -> str:
    ...
    fix, and a generic message sends someone to the wrong one (DIVERGER_PLAN.md §15's F-class
    error, in miniature).
    """
    code = e.status_code
    if code in (401, 403):
