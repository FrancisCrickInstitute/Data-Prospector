# Converger → Diverger conversion plan (rev. 50)
...
**What this does NOT resolve, and is the actual next step once a live run happens: whether DeepSeek's endpoint reports real cache numbers at all, or silently always reads 0.** If the compiler/worker prefixes (which carry Live Issue 31's ~7k-token `data_profile` addition, and are exactly the ones this measurement matters most for) never show a nonzero `cache_read` on repeat calls within a design's compile-retry loop, that's the answer to this entry's open question, and a materially different one from "caching works but nobody looked." **Needs a live run to actually read the numbers** — this entry stays open until one is.
...
**Logged as of rev. 50 (Live Issue 5), still not read from a live run.** `llm_call` now prints `usage.input_tokens`/`output_tokens`/`cache_read_input_tokens`/`cache_creation_input_tokens` on every successful call. A prefix below the provider minimum (1024 tokens Sonnet/Opus, 2048 Haiku) is silently ignored — no error, no saving. Ephemeral TTL is 5 minutes. See the Live Issue 5 entry for what this does and doesn't confirm yet, including the open question of whether DeepSeek's endpoint (worker/compiler, §5) reports real cache numbers at all.
