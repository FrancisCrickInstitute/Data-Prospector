# Converger → Diverger conversion plan (rev. 55)
...
| B5 | **A dtype gate that silently skips everything (Run 34).** `response-profile-archetypes` wrapped its entire Likert scale-detection in `if pd.api.types.is_object_dtype(series):`. The feedback CSVs load as pandas **`StringDtype`**, for which that returns `False`, so **all 21 string columns were skipped** and only the single `int64` satisfaction column reached the numeric branch. The clustering ran on one item — satisfaction tertiles — instead of the multi-item co-occurrence profile the angle described. | 34 | `pattern_not_shown`; the judge diagnosed it from two heatmap panels showing identical above/below-median lists, without seeing the code |
...
**Two observations worth keeping.** First, **the profile does print dtypes** — so if it renders `StringDtype` as `str` or `string`, the information was present and unused, whereas if it renders something friendlier the hint was never there; worth checking which, since it decides whether this is a profile-format fix or a genuinely new gap. Second, **this was a near-miss rather than a novel event**: Run 33's `likert-polarization-rank` did the same job on the same columns and survived only because it never asked about dtype at all, going straight to `.dropna().astype(str).str.strip()`. The difference between the two runs is one defensive call, not one insight — so expect recurrence.
