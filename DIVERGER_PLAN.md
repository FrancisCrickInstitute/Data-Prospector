...
**Live Issue 21 — WIDENED FIX LANDED, unconfirmed on a live run.** The Run 21 fix (raised validator budget, `realization_error` status, fifth gallery tier) was correct but too narrow: Run 22 hit the identical `max_tokens` error in the **worker fan-out**, upstream of the guard, and the angle was still mislabelled `not_realisable`. All four widening items from that diagnosis have now landed: `llm_call`'s default `max_tokens` is raised globally (8192 → 16384, so orchestrator and worker calls are covered along with everything else, not just the two callers that had an explicit override); the worker `gather` now uses `return_exceptions=True` with a per-function placeholder on failure instead of losing the whole design; `_run_one_design`'s entire body is wrapped in one try/except with a `stage` variable tracking which call is in flight, so a failure anywhere in the chain returns `realization_error` with whatever script/artifacts genuinely exist, and the log line says which stage broke instead of requiring log-bracket archaeology to find out. See §3's Live Issue 21 entry for the implementation detail. **Live Issue 22** (a script silently dropping a metric via a swallowed exception) landed alongside it, since both needed a Dockerfile rebuild. Needs a live run — see §3 for what to confirm.
