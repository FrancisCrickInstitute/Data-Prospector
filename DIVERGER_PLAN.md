# Converger → Diverger conversion plan (rev. 21)
...
**Live Issue 21 — FIXED, unconfirmed on a live run.** Run 21 lost two of its four realisations to a `max_tokens` failure inside `validate_realization`, and the failure was mislabelled in the gallery as a provisioning gap. All three parts of the fix have landed (see §3's Live Issue 21 entry) — a live run is needed to confirm the gallery's new fifth tier renders correctly and that a real `max_tokens` failure (if one recurs) no longer discards its script/artifacts. This was the last non-cleanup item; D-consolidate is next once this is confirmed.
...
`_write_gallery`/`_gallery_entry` verified standalone (synthetic angle data: one `realised`, one `realization_error` with a `requires` field and a real artifact/script, one `not_realisable` with its own `requires`) — confirmed the new tier renders the script/image, the `requires` field appears only under `not_realisable` and nowhere under the new tier, and the intro-sentence count picks up the new bucket correctly. Needs a live run to confirm the real thing: ideally one that reproduces a `max_tokens` failure (unlikely now at 16384, which is the point) or, failing that, one that confirms zero `realization_error` results and that the raised budget alone was sufficient without ever exercising the new status path for real.
...
**Ordering from here (rev. 21).** The D7 confirmation run is **done** (Run 21) and the functional programme with it. What is left:

1. **Live Issue 21** — the `max_tokens` failure in `validate_realization`, its mislabelling as a provisioning gap, and the discarded script. **FIXED, unconfirmed on a live run** (see §3) — all three parts landed and `_write_gallery`'s new fifth tier was verified standalone. **This was not cleanup**: it was a correctness bug in the oracle that cost Run 21 half its realisations and reported the loss under the wrong cause. Confirm on the next run before moving on.
