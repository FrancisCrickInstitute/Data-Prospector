# Converger → Diverger conversion plan (rev. 57)
...
| B5 | **A dtype gate that silently skips everything (Run 34).** `response-profile-archetypes` wrapped its entire Likert scale-detection in `if pd.api.types.is_object_dtype(series):`. The feedback CSVs load as pandas' native **`str`** dtype (pandas 3.0), for which that returns `False`, so **all 21 string columns were skipped** and only the single `int64` satisfaction column reached the numeric branch. The clustering ran on one item — satisfaction tertiles — instead of the multi-item co-occurrence profile the angle described. **Same shape as B4a, one dtype further in.** → Live Issue 33, **fixed rev. 56** (`AVAILABLE_LIBRARIES` now names the `str`/`object` split and points at `is_string_dtype()`), pending a live run to confirm | 34 | `pattern_not_shown`; the judge diagnosed it from two heatmap panels showing identical above/below-median lists, without seeing the code |
