...
**Closed/resolved/confirmed issues moved to `DIVERGER_PLAN_ARCHIVE.md` (rev. 55) to keep this section readable — nothing was deleted, only relocated.** What follows is the full text of every issue still open, then a one-line index of every closed one with a pointer to its full entry in the archive.
...
#### Closed — full text in `DIVERGER_PLAN_ARCHIVE.md`
...
## 16. Deferred: per-run token-usage summary, by model and stage

User-requested (rev. 67). Not started — nothing below is implemented; this is a to-do, parked the same way §9/§10 hold other not-yet-scheduled ideas.
...
- **Tokens only, or a cost estimate too?** A raw token count (by model, by stage) is cheap to keep accurate. A dollar figure needs a hardcoded $/token rate per model that goes stale the moment pricing changes and nobody notices — same "assumed vocabulary that goes stale" shape §15.7 observation 2 already warns about, just applied to pricing instead of data. If cost is wanted, prefer deriving it from a small, clearly-dated rate table over hand-asserting it once and forgetting it.
